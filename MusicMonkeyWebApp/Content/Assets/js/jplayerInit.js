...
var SingleJpPlayer = {
	init: function () {
		SingleJpPlayer.initSinglePlayer();

		setInterval(SingleJpPlayer.currentTimeAlign, 10);
		...
		$('.single_player').each(function () {
			...
			thisItem.find('.jp-next').click(function (e) { FastforwardTrack(); });
			thisItem.find('.jp-prev').click(function (e) { RewindTrack(); });
			...
			function RewindTrack() {
				...
				let currentProgress = GetPlayerProgress();
				let futureProgress = currentProgress - 5;
	...
	initSinglePlayer: function () {
		...
		players.each(function (e) {
			let player = $(this);
			let ancestor = player.data('ancestor');
			let songUrl = player.data('url');
			player.jPlayer({
				ready: function () {
	...
	currentTimeAlign: function () {
		$('.jp-progress').each(function () {
			let jpPBarW = $(this).find('.jp-play-bar').innerWidth();
			if (jpPBarW > 40) {
